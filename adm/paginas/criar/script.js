window.addEventListener('load', function() {
  ...
  function finalizarPag() {
    ...
    perguntasDiv.forEach((perguntaDiv, index) => {
        const pergunta = perguntaDiv.querySelector(`input[name^='pergunta${index + 1}']`).value;
        const opcao1 = perguntaDiv.querySelector(`input[name^='opcao${index + 1}1']`).value;
        const opcao2 = perguntaDiv.querySelector(`input[name^='opcao${index + 1}2']`).value;
        const opcao3 = perguntaDiv.querySelector(`input[name^='opcao${index + 1}3']`).value;
        const resposta = perguntaDiv.querySelector(`input[name^='resposta${index + 1}']`).value;

        if (pergunta.trim() === "" || opcao1.trim() === "" || opcao2.trim() === "" || opcao3.trim() === "" || resposta.trim() === "") {
